...
### Fixed
- 修复电脑休眠唤醒后手柄音频卡顿的问题
- 修复 USB 信号不稳定时手柄被误断开的问题
- 改善第三方手柄（如 imecoo）的连接兼容性（可能修复握手超时断开的问题）
...
### Fixed
- 修复了满载时的 mic 风收音问题
- 修复 Linux（PipeWire）下扬声器播放卡顿的问题
- 修复 Windows 休眠恢复后手柄可能无响应的问题
- 修复 FS 模式下 USB 音频 ISO 端点 bInterval 不匹配的问题
- 修复清除配对后重新配对时间过长的问题

### Added
- 新增电池电量显示（Create + Mute 查看，可配置常驻）
- 新增触控板全区域鼠标模式
- 新增触控板点击触发模式（需按下才触发方向键）
- 新增耳机音量独立偏移设置
- 新增音量锁定 4 级选项（关闭/禁止路由切换/锁定音量/完全锁定）
- 新增 D-pad 和触控板方向键重映射
- 新增 Create/Options 按键 release-to-send，防止组合键泄漏到游戏
- 新增音频转震动功能
- 新增 Opus 编码看门狗，自动恢复 DSP 卡死
- web 上开源版不显示 OTA 更新模块

### Changed
- 优化 Opus 编码性能（E907 P-extension SIMD 指令集优化）
- 优化蓝牙重连速度（5 秒 page-scan 优先窗口）
- 降低低电量闪烁阈值（10% → 0-9%）
- 启动延迟从 3 秒缩短到 1 秒
- 版本号集中管理（version.conf）
